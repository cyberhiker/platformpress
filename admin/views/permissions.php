...
if ( isset($_POST['role_name'] ) && wp_verify_nonce( $_POST['__nonce'], 'ap_role_'.$_POST['role_name'].'_update' ) && is_super_admin( ) ) {
	...
	if ( ! isset( $wp_roles->roles[ $role_slug ] ) ) {
		...
		$message = sprintf( __( 'New role %s added successfully .', 'platformpress' ), $role_name );
		$class .= ' notice notice-success';
	} else {
		$message = sprintf( __( 'Its look like %s role already exists .', 'platformpress' ), $role_name );
...
<div class="wrap">
	...
	<h2><?php _e('Permission and role', 'platformpress' ) ?></h2>
	<div class="white-bg">
	    <table class="form-table">
	        <tbody>
	       		<tr>
					<th scope="row" valign="top">
						<label><?php _e('Add new role', 'platformpress' ); ?>:</label>
					</th>
					<td>
						<p class="description"><?php _e('Add a new user role.', 'platformpress' );?></p>
						<br />
						<form action="" method="POST">
							<input type="text" name="role_name" value="" placeholder="<?php _e('Role name', 'platformpress' ); ?>" class="regular-text">
							<input type="text" name="role_slug" value="" placeholder="<?php _e('Role slug, without any space', 'platformpress' ); ?>" class="regular-text">
							<br />
							<br />
							<label>
								<input type="radio" name="role_caps" value="participant_caps">
								<?php _e('Basic capabilities', 'platformpress' ); ?>
							</label>

							<label>
								<input type="radio" name="role_caps" value="moderator_caps">
								<?php _e('Moderator capabilities', 'platformpress' ); ?>
							</label>
							<br />
							<br />
							<?php wp_nonce_field( 'ap_new_role', '__nonce' ); ?>
							<input name="new_role" type="submit" class="button button-primary" value="<?php _e('Add role', 'platformpress' ); ?>" />
						</form>
	            ...
	            <tr>
					<th scope="row" valign="top">
						<label><?php _e('PlatformPress capabilities', 'platformpress' );?>:</label>
						<p class="description"><?php _e('Add PlatformPress capabilities to 3rd party roles.', 'platformpress' );?></p>
					</th>

	                <td>
	                	<div class="ap-tools-roles">
							<label for="ap-tools-selectroles"><?php _e('Select user role', 'platformpress' ); ?></label>
							...
							<?php foreach ( $wp_roles->roles as $key => $role ) { ?>
								<form id="role_<?php echo $key; ?>" class="ap-tools-roleitem" style="display:none" method="POST" action="">
									...
									<div class="ap-tools-basecaps ap-tools-ck">
										<strong><?php _e( 'Basic Capabilities', 'platformpress' ); ?><input type="checkbox" class="checkall" /></strong>
									...
									<div class="ap-tools-modcaps ap-tools-ck">
										<strong><?php _e( 'Moderator Capabilities', 'platformpress' ); ?><input type="checkbox" class="checkall" /></strong>
