...
<div class="g-plusone" data-size="medium" data-annotation="inline" data-width="300" data-href="<?php echo $plankUrl; ?>"></div>
		...
		<div class="analysis">
		  ...
		  <li><i class="fa fa-eye"></i>
			...
			<?php if($this->settings['general']['is_user_logged_in']): ?>
				  <a title="My favorite" onClick="platformpressMarkQuestionFavorite(<?php echo $plankId; ?>)" href="javascript:void(0)" class="hear">
				  <i id="platformpress-fav-ques" class="fa fa-heart <?php echo ($this->isQuestionMarkedAsFavorite($plankId)) ? "marked-fav-ques" : "" ?>"></i>
				  </a>
			<?php endif; ?>
			</li>
  ...
  <script type="text/javascript">
	...
	function platformpressMarkQuestionFavorite(plankId){
		jQuery(document).ready(function(){
	...
	function platformpressMarkQuestionResolved(plankId,remarkId){
		jQuery(document).ready(function(){
			jQuery.ajax({
				method: 'POST',
	...
	<?php if($this->settings['general']['is_user_logged_in']): ?>
		...
		<div class="no-access">
			...
			Sorry, you are not allowed to remark planks
		</div>
		<?php }else{ ?>
		<div class="remark-heading"><h4>Answer this plank</h4></div>
